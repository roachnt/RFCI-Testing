import coverage
from steins_algorithm import gcd
import math
...
for i in range(1000):
    ...
    cov.start()
    val = gcd(a_list[i], b_list[i])
    if val == math.gcd(a_list[i], b_list[i]):
    ...
    for line_number in cov.get_data().lines(
            '/Users/nicklausroach/Classes/EECS493/RFCI_Testing/steins_algorithm/steins_algorithm.py'):
